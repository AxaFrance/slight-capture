...
export const detectAndMatch = (cv) => (imgDescription, im2, minMatch = 20) => {
   ...
   //const detectionAlgorithm = new cv.BRISK();
    ...
    // find the keypoints with ORB
    const {
        keypoints: keypointsObject,
        descriptors: descriptorsObject
    } = detectAndComputeSerializableToCv(cv)(imgDescription.descriptor, imgDescription.keyPoints);
...
export const computeHomography = (cv) => (imgDescription, sceneImg, minMatch = 20, inverseSourceAndDestination = false) => {
    ...
    const {
        ...
        goodMatches,
        clean
    } = detectAndMatch(cv)(imgDescription, sceneImg, minMatch);
